...
uint8_t g_UsbDeviceHidCompositeReportDescriptor[] = {
    /* Mouse descriptor */
    ...
    0x95U, 0x03U, /* Report Count (3U) */		//Kevin 3 bits, Crear un nuevo report descriptor que tenga un report id, pegar el report descriptor del mouse y pegar el report id 2 y el report del kwyboard
    0x75U, 0x01U, /* Report Size (1U) */		//KEvin tamaño de un 1 bit
...
{
#ifdef COMPOSITE_REPORT
    if (USB_HID_MOUSE_INTERFACE_INDEX == hidReportDescriptor->interfaceNumber)
    {
        hidReportDescriptor->buffer = g_UsbDeviceHidCompositeReportDescriptor;
        hidReportDescriptor->length = USB_DESCRIPTOR_LENGTH_HID_COMPOSITE_REPORT;
    }
#else
    if (USB_HID_MOUSE_INTERFACE_INDEX == hidReportDescriptor->interfaceNumber)
    {
        ...
        // Kevin hidReportDescriptor->buffer = g_UsbDeviceHidCompositeReportDescriptor;
        // Kevin hidReportDescriptor->length = USB_DESCRIPTOR_LENGTH_HID_COMPOSITE_REPORT;
    }
